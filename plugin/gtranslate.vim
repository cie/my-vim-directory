...
if exists("g:vtranslate")
	...
	vnoremap <silent> <plug>TranslateBlockText <esc>`<ma`>mb:call TranslateBlockText()<cr>`av`b"gp
...
class Translate
  ...
  def run(language, text)
    ...
    when /(.*):(.*)/ then
      from = $1
      to = $2
      @translator.translate(from.to_sym, to.to_sym, text)
    when /(.*)/ then
